...
class DataProcessor:
    ...
    def split_data(self, test_size: Optional[float] = 0.2, random_state: Optional[int] =42):
        ...
        train_set, test_set = train_test_split(self.df, test_size=test_size, random_state=random_state, stratify=self.df[self.config.target])
    ...
    def save_to_catalog(self, train_set: pd.DataFrame, test_set: pd.DataFrame, spark: SparkSession):
        ...
        train_set_with_ts = spark.createDataFrame(train_set) \
                                    .withColumn(
                                        "update_timestamp_utc", 
                                        to_utc_timestamp(current_timestamp(), 
                                        "UTC")
                                    )   
        
        test_set_with_ts = spark.createDataFrame(test_set) \
                                    .withColumn(
                                        "update_timestamp_utc", 
                                        to_utc_timestamp(current_timestamp(), 
                                        "UTC")
        ...
        train_set_with_ts.write \
                    .mode("append") \
                    .saveAsTable(f"{databricks_catalog}.{train_set_name}")
        
        logger.info('Saving test set to Databricks catalog')
        test_set_with_ts.write \
                    .mode("append") \
                    .saveAsTable(f"{databricks_catalog}.{test_set_name}")
        ...
        logger.info(f'Train/Test sets saved to location: {databricks_catalog} under {train_set_name} and {test_set_name}')
